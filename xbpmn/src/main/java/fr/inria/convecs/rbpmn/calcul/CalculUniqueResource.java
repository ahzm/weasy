package fr.inria.convecs.rbpmn.calcul;
...
public class CalculUniqueResource {
	...
	public Long getResourceUsage() {
		...
		traces.putAll(this.resource.getUsageTrace());
	...
	public Long getResourceUsage(Date afterDate) {
		...
		for(Date key : this.resource.getUsageTrace().keySet()) {
			if(key.getTime() >= afterDate.getTime()) {
				traces.put(key, this.resource.getUsageTrace().get(key));
			}
		}
